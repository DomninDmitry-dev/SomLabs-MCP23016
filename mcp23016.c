...
/*----------------------------------------------------------------------------*/
/*
 root@ubuntu:~$ cd /home/developer/source/somlabs-dts-1.0/
 root@ubuntu:/home/developer/source/somlabs-dts-1.0# dpkg-buildpackage -us -uc
 /usr/bin/make ARCH=arm -C /home/developer/source/kernel/linux-rel_imx_4.1.15_2.1.0_ga M=/home/developer/source/somlabs-dts-1.0
 * */
/*----------------------------------------------------------------------------*/
#define GPIO_NUM 16
#define INPUT 1
...
static irqreturn_t mcp23016_irq(int irq, void *data)
{
	...
	if(!mcp->toggleLed) {
		gpiod_set_value(mcp->led, 1);
		mcp->toggleLed = 1;
	}
	else {
		gpiod_set_value(mcp->led, 0);
		mcp->toggleLed = 0;
	}
                        ...
                        const struct i2c_device_id *id)
{
	...
	mcp->led = devm_gpiod_get(&client->dev, "led", GPIOD_OUT_HIGH);

	if (IS_ERR(mcp->led)) {
			dev_err(&client->dev, "%s() unable to get led GPIO: %ld\n",
					__func__, PTR_ERR(mcp->led));
			return PTR_ERR(mcp->led);
	}
	if(gpiod_direction_output(mcp->led, 0))
	{
